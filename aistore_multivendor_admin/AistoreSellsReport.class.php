...
$system_orders = $wpdb->prepare("SELECT * FROM {$wpdb->prefix}aistore_multivendor_system_orders WHERE product_owner_user_id=%d", $id);
$orders=$wpdb->get_results( $system_orders);
}
 else
 
{   $user=0;
   $orders=$wpdb->get_results( "SELECT * FROM {$wpdb->prefix}aistore_multivendor_system_orders ");
}
